          ...
          <v-list-item :key="index">
            <v-list-item-content>
              <v-list-item-subtitle>
                {{ inquiry.user.name }} / {{ $dateFns.format(new Date(inquiry.created_at), 'yyyy.MM.dd') }}
              </v-list-item-subtitle>
  ...
  export default {
    ...
    methods: {
      ...
      async getInquiriesPerPage(page){
        try{
          await this.$axios.get('/api/admin/inquiry/showAll', { params: { page: parseInt(page) } }) // /api/admin/inquiry/showAll?page=[page]になる
          .then((response) => {
      ...
      async fetchInquiries(){
        const response = await this.$axios.get('/api/admin/inquiry/showAll')
        .then((response) => {
          this.setInquiryInfo(response.data.inquiries.data);
          this.setPageInfo(response.data.inquiries);
        });
