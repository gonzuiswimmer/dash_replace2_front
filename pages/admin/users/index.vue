          ...
          <v-row>
            <v-col cols="8">
              <v-radio-group v-model="searchForm.status">
                <v-radio label="在籍" value="working"></v-radio>
                <v-radio label="退職済み" value="retired"></v-radio>
              </v-radio-group>
            </v-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async searchUser() {
        try {
          const response = await this.$axios.get('/api/admin/users',{ params: this.searchForm })
          .then((response) => {
      ...
      async getUsersPerPage(page){
        try{
          await this.$axios.get('/api/admin/users', { params: { page: parseInt(page) } }) // /api/admin/users?page=[page]になる
          .then((response) => {
            console.log(response);
            this.setUserInfo(response.data[0].data);
            this.setPageInfo(response.data[0]);
