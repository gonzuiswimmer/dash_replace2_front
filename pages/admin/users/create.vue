          ...
          <v-row>
            ...
            <v-col cols="8">
              <v-text-field
                ...
                label="パスワード確認"
                :rules="[rules.required, rules.password, rules.password_confirm]"
                :type="'password'"
              ></v-text-field>
            </v-col>
            ...
            <v-col cols="8">
              <div>
                入社日
                <date-picker v-model="registerUserForm.entry_date" type="date" lang="ja" format="YYYY-MM-DD"/>
              </div>
            </v-col>
...
<script>
  ...
  export default {
    ...
    data(){
      return {
        ...
        rules: {
          ...
          password: v => v.length >= 8 || 'パスワードは8文字以上で入力してください',
          email: v => /.+@.+/.test(v) || 'メールアドレス形式で入力してください',
          password_confirm: v => v === this.registerUserForm.password || 'パスワードが違います'
        },
      }
    },
    mounted(){
    ...
    methods: {
      async registerUser(){
        try {
          const response = await this.$axios.post('api/admin/users/store', this.registerUserForm )
          .then((response) => {
