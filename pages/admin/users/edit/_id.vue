        ...
        <v-form @submit.prevent="updateUser()">
          <v-row>
            ...
            <v-col cols="8">
              <div>
                入社日
                <date-picker v-model="updateUserForm.entry_date" type="date" lang="ja" format="YYYY-MM-DD" value-type="YYYY-MM-DD"/>
              </div>
            </v-col>
            ...
            <v-col cols="8">
              <div>
                誕生日
                <date-picker v-model="updateUserForm.birthday" type="date" lang="ja" format="YYYY-MM-DD" value-type="YYYY-MM-DD"/>
              </div>
            </v-col>
...
<script>
  ...
  export default {
    ...
    data(){
      return {
        ...
        assignSituations:[
          ...
          {
            text : 'アサイン中', value : true,
          },
          {
            text : '待機中', value : false,
          },
        ],
        blood_types:[
          {
    ...
    methods: {
      async updateUser(){
        try {
          const response = await this.$axios.patch(`api/admin/users/update/${this.updateUserForm.id}`, this.updateUserForm )
          .then((response) => {
      ...
      async fetchUserEditInfo(){
        try {
          await this.$axios.get(`/api/admin/users/edit/${this.$route.params['id']}`)
          .then((response) => {
            ...
            this.updateUserForm.department_id = response.data.user.department_id
            this.updateUserForm.gender = String(response.data.user.gender)
            this.updateUserForm.blood_type = response.data.user_profile.blood_type
            this.updateUserForm.birthday = response.data.user_profile.birthday ?? ''
            this.updateUserForm.github_url = response.data.user_profile.github_url ?? ''
            this.updateUserForm.qiita_url = response.data.user_profile.qiita_url ?? ''
            this.updateUserForm.self_introduction = response.data.user_profile.self_introduction ?? ''
